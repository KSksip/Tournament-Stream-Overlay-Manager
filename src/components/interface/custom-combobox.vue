<template>
    <div ref="root" class="relative" :class="isFocused ? 'z-50' : ''" >
        ...
        class="z-20 flex border whitespace-nowrap bg-white border-inherit w-full min-w-5">
            ...
            <input v-if="returnName" v-model="modelValue" @click="handleClickedInput()" :placeholder="props.placeholder" type="text" class="outline-none min-w-1 w-full grow placeholder:text-zinc-500">
            <input v-else v-model="modelValue.name" @click="handleClickedInput()" :placeholder="props.placeholder" type="text" class="outline-none min-w-1 w-full grow placeholder:text-zinc-500">
...
const filteredOptions = computed(() => {
    ...
    if(props.returnName){
        return props.options!.filter((option: any) => option.name.toLowerCase().includes(modelValue.value.toLowerCase()))
